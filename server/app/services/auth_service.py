...
class AuthService:
    ...
    def register_user(self):
        try:
            ...
            return Response.success_response({"user":{"id":user.id,"username":user.name,"email":user.email,"store_id":store.id,"created_at":user.created_at.isoformat()}}, "REGISTRATION_SUCCESSFUL"),201
    ...
    def login_user(self):
        ...
        return Response.success_response({"user":{"id":user.id,"username":user.name,"email":user.email,"store_id":store.id if store else None,"created_at":user.created_at.isoformat()}}, "LOGIN_SUCCESSFUL"),200
    def logout_user(self):
        ...
        return Response.success_response({"user":{"id":user.id,"username":user.name,"email":user.email,"created_at":user.created_at.isoformat()}}, "LOGOUT_SUCCESSFUL"),200
